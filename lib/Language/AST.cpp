...
/// Print a function argument, just the name and the type.
void ASTDumper::dump(ArgExprAST *node) {
  INDENT();
  llvm::errs() << "Arg: " << node->getName() << " " << loc(node) << "\n";
  dump(node->getType());
}
...
void ASTDumper::dump(const VarType &type) {
  llvm::errs() << "<" << type.shape << ", ";
  if (type.type == VarType::Type::si8) {
    llvm::errs() << "si8";
  } else if (type.type == VarType::Type::ui8) {
    llvm::errs() << "ui8";
  } else if (type.type == VarType::Type::si16) {
    llvm::errs() << "si16";
  } else if (type.type == VarType::Type::ui16) {
    llvm::errs() << "ui16";
  } else if (type.type == VarType::Type::si32) {
    llvm::errs() << "si32";
  } else if (type.type == VarType::Type::ui32) {
    llvm::errs() << "ui32";
  } else if (type.type == VarType::Type::f16) {
    llvm::errs() << "f16";
  } else if (type.type == VarType::Type::f32) {
    llvm::errs() << "f32";
  }
  llvm::errs() << ">";
}
