...
#ifndef DSPLANG_AST_H
#define DSPLANG_AST_H
...
/// Expression class for a function argument.
...
public:
  ArgExprAST(Location loc, llvm::StringRef name, VarType type,
             bool isPtr = false)
      : ExprAST(Expr_Arg, std::move(loc)), name(name), type(std::move(type)),
        isPtr(isPtr) {}
  ...
  /// LLVM style RTTI
  static bool classof(const ExprAST *c) { return c->getKind() == Expr_Arg; }
};
...
public:
  ...
  llvm::ArrayRef<std::unique_ptr<ArgExprAST>> getArgs() { return args; }
};
